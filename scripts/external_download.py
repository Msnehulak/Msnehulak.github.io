...
def external_download():
    ...
    for i in lib:
        ...
        if not _validate_file_name(file_name):
            logging.error(
                f"'{name}.{ftype}' contains invalid characters, please fix it in data/external_download.yaml"
            )
            sys.exit(1)
        ...
        if extra is not None and extra["save_as"]:
            ...
            if ".." in save_as:
                logging.error(
                    f"'{name} save as' contains invalid characters, please fix it in data/external_download.yaml"
                )
                sys.exit(1)
...
def move_file():
    if not DOWNLOAD_TO_PATH.exists():
        logging.warning(
            f"Path {DOWNLOAD_TO_PATH} don't exist.  maybe for got download?"
        )
        return
